...
const TARGETS = [
  ...
  { law_id:'358AC1000000032', group:'貸金業法', type:'法律' },     // 貸金業法（昭和58年法律第32号。ID6文字目は1）
  { law_id:'358CO0000000181', group:'貸金業法', type:'政令' },     // 貸金業法施行令
  { law_id:'358M50000040040', group:'貸金業法', type:'省令' },     // 貸金業法施行規則
  { law_id:'416AC0000000154', group:'信託業法', type:'法律' },     // 信託業法
  { law_id:'416CO0000000427', group:'信託業法', type:'政令' },     // 信託業法施行令
  { law_id:'412AC0000000101', group:'金サ法',   type:'法律' },     // 金融サービスの提供及び利用環境の整備等に関する法律
  { law_id:'412CO0000000484', group:'金サ法',   type:'政令' },     // 同 施行令
  ...
  { law_id:'324AC0000000228', group:'外為法',   type:'法律' },     // 外国為替及び外国貿易法（AML/CFT・経済制裁）
  { law_id:'355CO0000000260', group:'外為法',   type:'政令' },     // 外国為替令
...
// 直後が単独「一」のとき、その「一」が数詞でなく熟語(〜いつ)の一部になる接頭字。
// 同一(どういつ)/唯一/単一/統一/均一/画一/純一 等。例: 同一人→「同1人」化を防ぐ(同一の人物)。
const ONE_WORD_PREV = '同唯単統均画純';
...
function kanjiNum(text){
  ...
  let out = text.replace(re, (m, off, str) => {
    ...
    // 熟語の「一」(同一・唯一・単一…)は数詞でないので守る（同一人→「同1人」化を防ぐ）
    if (m === '一' && ONE_WORD_PREV.includes(prev)) convert = false;
...
function main(){
  ...
  // グループ順を固定
  const G = ['金商法','資金決済','犯収法','銀行法','会社法','個人情報','ガイドライン'];
